...
export default function Level2Page() {
  ...
  // Fetch Level 2 settings from Firestore via API
  useEffect(() => {
  ...
  const handleNextLevel = async () => {
    ...
    // Update user level to 3 if not already
    if (user && user.currentLevel < 3) {
      try {
        await fetch('/api/user/level', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ uid: user.uid, level: 3 }),
        })
      } catch (err) {
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 bg-gray-950 border-b border-gray-800">
              <div>
                <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-widest">Level 2 · Review Video</p>
            ...
            <div className="px-4 py-2 bg-gray-800 border-b border-gray-700 flex flex-wrap gap-x-5 gap-y-1 items-center">
              <span className="text-[11px] text-gray-400">
                <span className="text-yellow-400 font-semibold">Password: </span>
                <span className="font-mono text-yellow-300 select-all">{settings?.videoPassword ?? '...'}</span>
              </span>
              {(settings?.formFields ?? []).map((f) => (
                <span key={f.label} className="text-[11px] text-gray-400">
                  <span className="text-gray-300 font-semibold">{f.label}: </span>
                  <span className="font-mono text-gray-200 select-all">{f.value}</span>
                </span>
              ))}
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 justify-center w-full max-w-7xl mx-auto">
          ...
          <div className="lg:w-[30%] w-full">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-1">Level 2 – Reviewer</h2>
