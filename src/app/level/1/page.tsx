...
interface Level1Settings {
  minDocsToPass: number
  totalDocs: number
  requirements: Level1Requirement[]
}

// ─── Default fallback (matches original hardcoded values) ────────────────────

const DEFAULT_SETTINGS: Level1Settings = {
  minDocsToPass: 4,
  totalDocs: 7,
  requirements: [
...
export default function Level1Page() {
  ...
  const [levelSettings, setLevelSettings] = useState<Level1Settings>(DEFAULT_SETTINGS)

  useEffect(() => {
    // Load admin-configured requirements
    fetch('/api/admin/level1-settings')
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data) setLevelSettings(data) })
      .catch(() => { /* use defaults */ })
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChange((userData) => {
      ...
      if (userData.status === 'rejected') { setError('Your account has been rejected. Please contact support.'); setLoading(false); return }
      if (!userData.profileCompleted) { router.push('/complete-profile'); return }
      if (userData.currentLevel < 1) { router.push('/dashboard'); return }
  ...
  // ─── Derived values (now use dynamic settings) ────────────────────────────

  const { minDocsToPass, requirements } = levelSettings
  ...
  const handleIntroCheck = async (checked: boolean) => {
    if (!user) return
    if (!checked && user.currentLevel >= 2) return
    ...
    if (checked) {
      try {
        ...
        setUser({ ...user, requirementsCompleted: [...user.requirementsCompleted, 'read_intro'] })
      } catch (error) {
        console.error('Failed to save intro progress:', error)
        setError('Failed to save progress. Please try again.')
      }
    } else {
      try {
        await removeRequirementComplete('read_intro')
        setIntroChecked(false)
        setUser({ ...user, requirementsCompleted: user.requirementsCompleted.filter(r => r !== 'read_intro') })
      } catch (error) {
        console.error('Failed to remove intro progress:', error)
        setError('Failed to remove progress. Please try again.')
      }
    }
  }
  ...
  const markRequirementComplete = async (requirementId: string) => {
    ...
    try {
      ...
      if (user) setUser({ ...user, requirementsCompleted: [...user.requirementsCompleted, requirementId] })
    } catch (error) {
      console.error('Error updating progress:', error)
    }
  }

  const removeRequirementComplete = async (requirementId: string) => {
    if (!user) return
    try {
      const response = await fetch('/api/user/requirement', {
        method: 'DELETE',
      ...
      if (!response.ok) throw new Error('Failed to remove progress')
      if (user) setUser({ ...user, requirementsCompleted: user.requirementsCompleted.filter(r => r !== requirementId) })
    } catch (error) {
  ...
  const updateUserLevel = async (newLevel: number) => {
    try {
      const response = await fetch('/api/user/level', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: user?.uid, level: newLevel }),
      })
      if (!response.ok) throw new Error('Failed to update level')
      if (user) setUser({ ...user, currentLevel: newLevel })
    } catch (error) {
  ...
  const handleNextLevel = async () => {
    ...
    try {
      ...
      console.error('Failed to proceed to next level:', err)
  ...
  const viewDocument = (url: string, fileName: string) => {
    ...
    if (!docExists) { setError('Document no longer available'); return }
  ...
  const handleDocumentUpload = async (docType: string, file: File | null) => {
    ...
    setPendingFiles(prev => [...prev.filter(p => p.type !== docType), { type: docType, file: renamedFile, previewUrl }])
    setProgressSaved(false)
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      if (pendingFiles.length > 0) {
        ...
        for (const pending of pendingFiles) {
          const url = await uploadMultipleToCloudinary(
            [pending.file], 'mru-roadmap', user.uid, user.name || user.displayName, pending.type
          )
          currentDocs = [
            ...currentDocs.filter(d => d.type !== pending.type),
            { type: pending.type, fileName: pending.file.name, url: url[0], level: 1, uploadedAt: new Date().toISOString() },
          ]
      ...
      // ✅ Single save call (was being called twice before)
      await saveUploadedDocuments(currentDocs)
      setUploadedDocuments(currentDocs)

      // Persist intro check
      if (introChecked && !user.requirementsCompleted.includes('read_intro')) {
        await markRequirementComplete('read_intro')
      }

      if (currentDocs.length >= minDocsToPass) await markRequirementComplete('documents_uploaded')
      ...
      if (res.ok) {
        ...
        for (const dbDoc of dbDocs.filter((d: any) => d.level === 1)) {
          if (!currentDocs.find(d => d.type === dbDoc.type)) {
      ...
      setUploadingDocuments(false)
      throw error  // ✅ Re-throw so handleNextLevel catches it and doesn't route
    }
  }
  ...
  const removeDocument = async (docType: string) => {
    try {
      ...
      try {
        ...
        const imageRes = await fetch('/api/cloudinary/delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ publicId, resourceType: 'image' }) })
        const imageResult = await imageRes.json()
        if (imageResult.result === 'ok') {
          cloudinaryDeleted = true
        } else {
          const rawRes = await fetch('/api/cloudinary/delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ publicId, resourceType: 'raw' }) })
          const rawResult = await rawRes.json()
          if (rawResult.result === 'ok') cloudinaryDeleted = true
        }
      } catch (cloudinaryError) {
        console.warn('Cloudinary deletion failed:', cloudinaryError)
      }

      const deleteRes = await fetch('/api/user/documents', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: user?.uid, type: docType, level: 1 }),
      })
      if (!deleteRes.ok) throw new Error('Failed to delete from database')

      setUploadedDocuments(uploadedDocuments.filter(d => d.type !== docType))
      setProgressSaved(false)
      setRemoveConfirm(false)
      setSuccess(cloudinaryDeleted ? 'File deleted successfully!' : 'File removed from records (Cloudinary cleanup may be pending).')
      setTimeout(() => setSuccess(''), 3000)
      if (user) await loadUploadedDocuments(user)
    } catch (error) {
      console.error('Error removing document:', error)
      setRemoveConfirm(false)
      setSuccess('Failed to delete file. Please try again.')
      setTimeout(() => setSuccess(''), 3000)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-amber-100">
      ...
      <main className="w-[90%] mx-auto px-4 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 justify-center w-full max-w-7xl mx-auto">
          ...
          <div className="lg:w-[30%] w-full">
            <div className="mb-4 flex items-center gap-2">
              <button onClick={() => router.push('/dashboard')} className="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-700 border border-gray-300 rounded-lg font-semibold hover:bg-gray-200 transition-colors text-sm">
                Dashboard
              </button>
              <button
                onClick={handleNextLevel}
                disabled={!canProceedToNext}
                className={`px-4 py-2 rounded-lg font-semibold transition-colors text-sm ${canProceedToNext ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
              >
                Next Level
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  {showJourney && (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700">This roadmap consists of 7 carefully designed levels, each building upon the previous one.</p>
                      </div>
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">Why Choose This Career?</h4>
                        <p className="text-gray-700">Financial Advisors play a crucial role in helping individuals and families achieve their financial goals.</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t pt-3">
                  <label className={`flex items-start space-x-3 ${user!.currentLevel >= 2 ? 'cursor-default opacity-70' : 'cursor-pointer'}`}>
                    <input
                      type="checkbox"
                      checked={introChecked}
                      onChange={(e) => handleIntroCheck(e.target.checked)}
                      disabled={introChecked && user!.currentLevel >= 2}
                      className="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 disabled:opacity-60"
                    />
                    <span className="text-sm text-gray-700">
                      I have read and understand the introduction content
                      {introChecked && user!.currentLevel >= 2 && (
                        <span className="ml-2 text-xs text-green-600 font-medium">(locked)</span>
                      )}
                    </span>
                  </label>
                </div>
          ...
          {/* ── Right Side — Requirements Checklist ── */}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold transition-colors text-sm ${progressSaved ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                >
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-700">
                    Documents Uploaded: {uploadedDocuments.length}/{totalDocs}
                  </span>
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-gray-600">Overall Progress:</span>
                    <span className={`text-sm font-bold ${levelComplete ? 'text-green-600' : 'text-blue-600'}`}>
                      {progressPercentage}%
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-sm font-medium ${progressSaved ? 'text-green-600' : 'text-orange-600'}`}>
                ...
                <div className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`w-4 h-4 rounded-full ${isIntroRead ? 'bg-green-500' : 'bg-gray-300'}`} />
                  ...
                  <p className="text-sm text-gray-600 mt-2 ml-7">Check the box in the introduction section after reading</p>
                </div>
                ...
                {requirements.map((doc) => (
                  <div key={doc.type} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        <div className={`w-4 h-4 rounded-full ${uploadedDocuments.find(d => d.type === doc.type) ? 'bg-green-500' : 'bg-gray-300'}`} />
                    ...
                    {/* Sample Images — rendered from admin settings */}
                    ...
                    <div>
                      <input
                        ...
                        disabled={uploadingDocuments === 'saving' || Boolean(uploadedDocuments.find(d => d.type === doc.type))}
                      />
                      ...
                      {!uploadedDocuments.find(d => d.type === doc.type) && pendingFiles.find(p => p.type === doc.type) && (
                        <div className="mt-2 flex items-center justify-between">
                          ...
                          <button onClick={() => setPendingFiles(prev => prev.filter(p => p.type !== doc.type))} className="text-red-400 hover:text-red-600 text-xs">
                            Cancel
                          </button>
                        </div>
                      )}
                      ...
                      {uploadedDocuments.find(d => d.type === doc.type) && (
                        <div className="mt-2 flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => viewDocument(uploadedDocuments.find(d => d.type === doc.type)?.url || '', uploadedDocuments.find(d => d.type === doc.type)?.fileName || '')}
                              className="text-blue-500 hover:text-blue-700 text-xs font-medium"
                            >
                              View
                            </button>
                            <button onClick={() => setRemoveConfirm(doc.type)} className="text-red-500 hover:text-red-700 text-xs font-medium">
                              Remove
                            </button>
                          </div>
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
              ...
              <p className="text-sm text-gray-700 mb-6">
                ...
                <span className="font-semibold">
                  {requirements.find(d => d.type === removeConfirm)?.name || removeConfirm}
                </span>?
              </p>
              <div className="flex space-x-3">
                <button onClick={() => setRemoveConfirm(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
                <button onClick={() => removeDocument(removeConfirm as string)} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors">
                  Yes, Remove
                </button>
              </div>
            </div>
        ...
        {viewingDoc && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-3 sm:p-4" onClick={(e) => { if (e.target === e.currentTarget) setViewingDoc(null) }}>
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden" style={{ height: '90vh' }}>
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 flex-shrink-0">
                <p className="text-sm font-semibold text-gray-800 truncate flex-1 pr-4">{viewingDoc.fileName}</p>
                <button onClick={() => setViewingDoc(null)} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 flex-shrink-0">
              ...
              <div className="flex-1 overflow-auto bg-gray-100">
                {viewingDoc.fileName.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                  <div className="flex items-center justify-center min-h-full p-4">
                    <img src={viewingDoc.url} alt={viewingDoc.fileName} className="max-w-full max-h-full object-contain rounded-lg shadow-md" />
                  </div>
                ) : (
                  <iframe src={`https://docs.google.com/viewer?url=${encodeURIComponent(viewingDoc.url)}&embedded=true`} className="w-full h-full" title={viewingDoc.fileName} />
                )}
              </div>
              <div className="px-4 py-3 border-t border-gray-200 flex-shrink-0">
                <button onClick={() => setViewingDoc(null)} className="w-full py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors text-sm">
                  Close
                </button>
              </div>
            </div>
